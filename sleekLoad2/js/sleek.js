...
function SetStatusChanged( status ) {
	if (status.indexOf(" Addonslar yükleniyor  #") != -1) {
		downloadedFiles++;
		refreshProgress();
	}else if (status == "Cok az kaldı") {
		setProgress(100);
	}
...
function setProgress(progress) {
	$("#yükleniyor").css("width", progress + "%");
}

function setGamemode(gamemode) {
	$("#oyun modu").html(gamemode);
}
...
function setMusicName(name) {
	$("#muzik ismi").fadeOut(2000, function() {
		$(this).html(name);
		$(this).fadeIn(2000);
	});
}
